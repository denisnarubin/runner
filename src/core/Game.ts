...
export class Game {
    ...
    private createFinishLine(): void {
        ...
        const tilesZ = 3;
        const tilesX = Math.ceil(roadWidth / tileSize);
    ...
    private checkCollisions(): void {
        ...
        for (let i = objects.length - 1; i >= 0; i--) {
            ...
            // 🔥 ИСПРАВЛЕНО: Добавлена проверка видимости объекта
            // Объект должен быть видимым для сбора
            ...
            case 'bomb':
                ...
                if (this.player) {
                    this.player.playFall(() => {
                        ...
                        setTimeout(() => this.showEndCard(false), 500);
                    });
                }
    ...
    private async playSound(name: string): Promise<void> {
        try {
            ...
            // 🔥 ИСПРАВЛЕНО: Для gate используем оригинальный audio, а не clone,
            // чтобы избежать проблем с загрузкой
            if (name === 'gate') {
                audio.currentTime = 0;
                audio.play().catch(err => console.warn(`⚠️ Ошибка воспроизведения ${name}:`, err));
            } else {
