...
export class Game {
    ...
    constructor(canvas: HTMLCanvasElement) {
        ...
        console.log(`🏁 Финишная прямая на Z = ${this.finishLineZ}`)
    }
    ...
    private showTutorial(): void {
        ...
        overlay.innerHTML = `
            ...
            <p style="font-size: 18px; margin: 10px;">🔷 Проходи сквозь <b>голубые ворота</b> для бонусов!</p>
            <p style="font-size: 18px; margin: 10px;">🏁 Достигни <b>финиша</b> и затанцуй!</p>
    ...
    private handleFinish(): void {
        ...
        console.log('🏁 ФИНИШ! Персонаж пересек линию!')
        ...
        if (this.player && !this.isGameOver) {
            console.log('🔄 Поворачиваем персонажа к камере')
            this.player.faceCamera()
            setTimeout(() => {
                if (this.player && !this.isGameOver) {
                    console.log('💃 Запускаем Breakdance!')
                    this.player.playBreakdance(() => {
                        console.log('🎉 Танец закончен, показываем экран победы')
                        setTimeout(() => {
                            this.endGame(true)
                        }, 1500)
                    })
    ...
    private handleCollision(obj: THREE.Object3D, objPos: THREE.Vector3): void {
        ...
        switch (type) {
            ...
            case 'gate':
                // 🔥 Работа с парными воротами
                const gate = (obj as any).gate as Gate
                const playerX = this.player?.getPosition().x || 0
                
                // 🔥 Определяем, в какую сторону вошёл игрок
                const side: 'left' | 'right' = playerX < 0 ? 'left' : 'right'
                const modifier = gate?.getModifier(side)
                
                if (modifier) {
                    const modType = modifier.type
                    const value = modifier.value
                    console.log(`🚪 Проходим через ${side} сторону ворот: ${modType}${value}`)
                    
                    if (modType === '+') {
                        this.score += value
                        console.log(`➕ Добавлено ${value} монет. Теперь: ${this.score}`)
                    } else if (modType === '-') {
                        this.score = Math.max(0, this.score - value)
                        console.log(`➖ Вычтено ${value} монет. Теперь: ${this.score}`)
                    } else if (modType === 'x') {
                        this.score *= value
                        console.log(`✖️ Умножено на ${value}. Теперь: ${this.score}`)
                    }
                    this.createGateEffect(objPos)
                }
                this.world?.removeObject(obj)
                this.playSound('gate')
                break
    ...
    private createGateEffect(position: THREE.Vector3): void {
        const light = new THREE.PointLight(0x33aaff, 3, 10)
        light.position.copy(position)
        this.scene.add(light)
        setTimeout(() => {
            this.scene.remove(light)
